...
// Ported and modified from http://z80ex.sourceforge.net/ by toptensofrware
// ...then ported and modified from https://github.com/toptensoftware/yazd by Max Samoylov
//
...
class OpCode
{
    constructor (mnemonic, t_states, t_states2, flags = OpCodeFlags.Continues)
    {
        ...
        if (mnemonic != null)
        {
			...
			// uncomment it if you want something like "XOR A,C" instead of "XOR C"
            /*if ((this.flags & OpCodeFlags.ImplicitA) != 0)
            {
                var spacePos = this.mnemonic.indexOf(' ');
                if (spacePos >= 0)
                {
                    this.mnemonic = mnemonic.substring(0, spacePos + 1) + "A," + mnemonic.substring(spacePos + 1);
                }
                else
                {
                    this.mnemonic = mnemonic + " A";
                }
            }*/
        }
	}
	getCopy(){
		return {
        	...
        	t_states: this.t_states,
        	t_states2: this.t_states2,
        	flags: this.flags
		};
	}
}

// Disassembles Z80 opcodes
// fn_readbyte - function like fn_readbyte(address) that returns one byte from the ZX Spectrum memory at address
//               your function have to handle wrong addresses like less than 0x0000 and greater than 0xffff
//
// .process(address) -  processes one opcode and returns an object:
//  --->{
//          mnemonic: string like "RET Z" or null
//			comment: for relative jumps etc.
//          ticks1: number of T-states;
//          ticks2: number of T-states for operaions with various behavior like "RET Z";
//          flags: number. Flags of OpCode class member (for compatibility with ported version, not necessary)
//          bytesLength: number - opcode length in bytes. For example - "LD HL, 0xffff" takes 3 bytes, so it will be 3;
//      }
class Z80Disassembler
{
    constructor(fn_readbyte)
    {
		if (typeof fn_readbyte != 'function') throw("new Z80Disassembler(fn) requires a function as it's parameter");
        this.readByte = fn_readbyte;

        this.base = this.dasm_base();
        this.shift_CB = this.dasm_cb();
        this.shift_DD = this.dasm_dd();
        this.shift_ED = this.dasm_ed();
        this.shift_FD = this.dasm_fd();
        this.shift_DDCB = this.dasm_ddcb();
        this.shift_FDCB = this.dasm_fdcb();
    }
    ...
    process(address)
    {
		...
		var opcode = this.base[this.readByte(address)].getCopy();
		
		var doublePrefix = false;  													// will increase opcode length at exit

        if (opcode.mnemonic && opcode.mnemonic.indexOf("shift") != -1) {
            // read from this.shift_**
            result.bytesLength++;                                           		// length atleast = 2 (DD op ...)
            opcode = this[opcode.mnemonic][this.readByte(address + 1)].getCopy();

            if (opcode.mnemonic && opcode.mnemonic.indexOf("shift") != -1) {
				// this.shift_DDCB and this.shift_FDCB
				doublePrefix = true;
                opcode = this[opcode.mnemonic][this.readByte(address + 3)].getCopy(); // NOT +2
            }
		}

        // catch and replace symbols
        // @	2 byte	Low High
		// #	1 byte	Low
		// $	1 byte	relative to IX or IY
		// %	1 byte	The jump is measured from the address of the instruction op code and contains a range of –126 to +129 bytes.
		var lowbyte;

        if (opcode.mnemonic && opcode.mnemonic.indexOf("@") != -1) {
			lowbyte = this.readByte(address + result.bytesLength);
			var highbyte = this.readByte(address + result.bytesLength + 1);
            opcode.mnemonic = opcode.mnemonic.replace(
                "@",
                this.getHexString(lowbyte, highbyte)
			);
			//result.comment = "0b" + (lowbyte + highbyte * 256).toString(2).padStart(16, "0");
            result.bytesLength += 2;
		}
		...
		if (opcode.mnemonic && opcode.mnemonic.indexOf("$") != -1) {
            ...
            opcode.mnemonic = opcode.mnemonic.replace(
				"$",
				"0x" + (Number(lowbyte)).toString(16).padStart(2, "0").toUpperCase()
			);
			result.comment =  (lowbyte > 127 ?
				"-" + "0x" + Math.abs((Number(lowbyte) - 256)).toString(16).padStart(2, "0").toUpperCase() :
				"+" + "0x" + (Number(lowbyte)).toString(16).padStart(2, "0").toUpperCase()
			);
			result.bytesLength++;
		}
        ...
        if (opcode.mnemonic && opcode.mnemonic.indexOf("#") != -1) {
			...
			//result.comment = "0b" + lowbyte.toString(2).padStart(8, "0");
            result.bytesLength++;
		}
        ...
        if (opcode.mnemonic && opcode.mnemonic.indexOf("%") != -1) {
			...
			var jumpAddress = (lowbyte > 127 ? address + (lowbyte + 2) - 256 : address + (lowbyte + 2));
			result.comment = "to " + this.getHexString(jumpAddress);
            result.bytesLength++;
        }

        result.mnemonic = opcode.mnemonic;
        result.ticks1 = opcode.t_states;
        result.ticks2 = opcode.t_states2;
		result.flags = opcode.flags;
		if (doublePrefix) result.bytesLength++;

        return result;
    }

    // returns byte(s) as hexadecimal string
    getHexString(lowbyte, highbyte = 0)
    {
        return "0x" + (Number(highbyte) * 256 + Number(lowbyte)).toString(16).padStart(4, "0").toUpperCase();
    }
